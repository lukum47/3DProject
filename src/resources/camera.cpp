...
Camera::Camera(const unsigned int& scrWidth, const unsigned int& scrHeight, glm::vec3 position, glm::vec3 up, GLfloat yaw, GLfloat pitch) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), movementSpeed(SPEED), mouseSensitivity(SENSITIVITY), zoom(ZOOM)
{
	lastX = scrWidth / 2.f;
	lastY = scrHeight / 2.f; 
...
void Camera::processMouseMovement(GLfloat xoffset, GLfloat yoffset, GLboolean constrainPitch)
{
	if (firstMouse)
	{
		...
		float xpos = xoffset - lastX;
		float ypos = lastY - yoffset;
		lastX = xoffset;
		lastY = yoffset;

	xpos *= this->mouseSensitivity;
	ypos *= this->mouseSensitivity;
